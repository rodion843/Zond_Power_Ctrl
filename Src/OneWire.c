...
void usart_setup_(uint32_t baud) {									 
	...
	__HAL_UART_ENABLE_IT(&huart3, UART_IT_RXNE); // разрешаем прерывание по приему от UART1
}
...
void owReadHandler() { //обработчик прерыания USART
	...
	if (((OW_USART->CR1 & USART_CR1_RXNEIE) != 0) &&
			((OW_USART->ISR & UART_FLAG_RXNE) != (uint16_t)RESET)) { // SR->ISR
		while ((OW_USART->ISR & UART_FLAG_RXNE) == (uint16_t)RESET){;}	  /* Получаем данные из периферии и сбрасываем флаг*/
		rc_buffer[index] = (uint16_t)(OW_USART->RDR & (uint16_t)0x01FF);	// DR->RDR
...
void owSend(uint16_t data) {
	...
	OW_USART->TDR = (data & (uint16_t)0x01FF);	//DR->TDR SR->ISR
	while ((OW_USART->ISR & UART_FLAG_TC) == (uint16_t)RESET){;}	  //ждем пока передача закончится
}
